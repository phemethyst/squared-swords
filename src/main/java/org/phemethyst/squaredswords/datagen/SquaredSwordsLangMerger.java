 ...
 * {@code assets/squaredswords/lang/default/*.json}, and only the translation <em>keys</em>
 * appear in code (see {@link org.phemethyst.squaredswords.Lang}).
 *
 ...
 * into that same provider via {@link SquaredSwords#registerLangPartials()}, so the partials
 ...
 */
public class SquaredSwordsLangMerger {

    private static final Gson GSON = new Gson();

    /** Partial file names under {@code assets/squaredswords/lang/default/}, without ".json". */
    ...
    public static void mergeInto(BiConsumer<String, String> consumer) {
        for (String partial : PARTIALS) {
            String path = "assets/" + SquaredSwords.ID + "/lang/default/" + partial + ".json";
            try (InputStream in = SquaredSwordsLangMerger.class.getClassLoader().getResourceAsStream(path)) {
                if (in == null) {
                    SquaredSwords.LOGGER.error("Lang partial not found on the classpath: {}", path);
                    continue;
                }

...
- **SU-consuming kinetic block + goggle overlay** — `content/kinetics/SquaredSwordsKineticBlock` +
  `SquaredSwordsKineticBlockEntity`. An encased-shaft-style block that joins the kinetic
  ...
  Its model is an inset casing and `SquaredSwordsShaftRenderer` spins a Create shaft that pokes
...
- **Kinetic generator + goggle overlay** — `content/kinetics/SquaredSwordsGeneratorBlock` +
  `SquaredSwordsGeneratorBlockEntity`. The counterpart source: it *adds* stress capacity
  ...
  network and nothing turns. It shares `SquaredSwordsShaftRenderer` and adds custom lines to the
  Engineer's Goggles tooltip (`IHaveGoggleInformation#addToGoggleTooltip`).
- **Display Link source** — `content/display/SquaredSwordsDisplaySource` + `AllDisplaySources`.
...
- **Sequenced assembly recipe** — `datagen/SquaredSwordsSequencedAssemblyGen`. An item is
  processed through a Deployer then a Mechanical Press, looping twice, using a
  transitional "incomplete" item.
- **Ponder plugin** — `content/ponder/SquaredSwordsPonderPlugin` +
  `SquaredSwordsPonderScenes`. Wires up Create's in-game animated manual for the addon and
  is registered client-side in `SquaredSwords#onClientSetup`. Ships a worked scene driven
  by the `assets/squaredswords/ponder/squaredswords_ponder.nbt` schematic (two Mechanical Arms and
  ...
  (`SquaredSwords#registerPonderLang`), so it appears after `./gradlew runData`.

- **Create-style item tooltip** — the `squaredswords_item`'s tooltip is authored as lang keys in
  `assets/squaredswords/lang/default/tooltips.json` (`.tooltip.summary` +
  `.tooltip.condition1`/`.behaviour1`). The `ItemDescription` modifier set up on the
  Registrate in `SquaredSwords` reads them and adds the "Hold Shift" prompt and highlight
...
- **Lang: keys in code, copy in JSON** — like Create, English strings live in hand-authored
  partials under `assets/squaredswords/lang/default/` (`interface.json`, `tooltips.json`), while
  `Lang` holds only translation keys. `datagen/SquaredSwordsLangMerger` merges the partials into the
  ...
  `SquaredSwordsLangMerger`) — never by putting English in Java.
- **Additional languages** — `assets/squaredswords/lang/de_de.json` is a worked German
...
It clones this template and rebrands every `Example` / `squaredswords` reference to your
...
Run the bootstrap task once — it rewrites every `Example` / `squaredswords` reference,
...
2. Rename `src/main/java/com/example/squaredswords/` to match your `mod_group_id`
3. Update `SquaredSwords.java` — change `ID` to your `mod_id`
4. Rename `src/main/resources/squaredswords.mixins.json` to `{mod_id}.mixins.json` and
...
1. Go into your SquaredSwords.java class to get a quick overview over what is defined and where exactly.
